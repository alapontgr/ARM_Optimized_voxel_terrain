...
void DrawL0(InnerParams* params) {
  ...
  Pixel* line = params->line;
  unsigned int* world_line = params->w_line;
  unsigned int* inv_diff_y = params->inv_diff_y; 
  int* last_color_ptr = params->last_color;
  int last_color = *last_color_ptr;
  int* last_y_ptr = params->last_y; 
  int last_y = *last_y_ptr;
  int y;
  for(y = 0; y < level_2 && last_y < h; ++y) {
      ...
      if(current_y > last_y) {
        ...
        current_y = GetMin(current_y, h - 1);
        int y0;
...
static void RotateScreen(Pixel* world_final, Pixel* screen) {
  ...
  for(y = 0; y < num_tiles; ++y) {
    int x;
    //unsigned int* src = &world_final[y * SCR_WIDTH];
    for(x = 0; x < num_tiles; ++x) {
      ...
      //screen[y + ((SCR_HEIGHT-x-1) * SCR_WIDTH)] = src[x];
    }
  }
}
