...
#define SCR_WIDTH_2   512
#define SCR_HEIGHT    1024
#define SCR_HEIGHT_2  512
#define MAX_STEPS     2048
#define MAX_STEPS_2   1024
...
static void DrawTerrain(DrawTerrainParams* params) {
  unsigned int* scr = params->scr;
  int level_2 = params->level_2;
  unsigned int* palette = params->palette;
  ...
  for(x = 0; x < SCR_WIDTH; ++x) {  
    ...
    unsigned int* line = &scr[x * SCR_WIDTH];
    for(y = 0; y < level_2 && last_y < SCR_HEIGHT; ++y) {
      unsigned int texel = world_line[y];
      ...
      if(current_y > last_y) {
        ...
        int y0;     
        for(y0 = last_y; y0<=current_y-1; y0++) {
          unsigned int f = color>>12; 
          line[y0] = palette[f] | (current_y >> 2);
          color += delta_color;
        } 
        last_y = current_y;
...
static void RotateScreen(unsigned int* world_final, unsigned int* screen) {
  int y;
    ...
    for(x = 0; x < num_tiles; ++x) {
      ...
      for(tile_y = 0; tile_y<tile_size; tile_y++) {
        ...
        for(tile_x = 0; tile_x<tile_size; tile_x++) {
          screen[y0 + ((SCR_HEIGHT - x0 -1) * SCR_WIDTH)] = world_final[y0 * SCR_WIDTH + x0];
          x0++;
        }
        y0++;
      }
...
int main(int argc, char** argv) {
  ...
  unsigned int* world_final = (unsigned int*)malloc(SCR_WIDTH * MAX_STEPS * 4);
  memset(world, 0, SCR_WIDTH * MAX_STEPS * 4);
  memset(world2, 0, SCR_WIDTH_2 * MAX_STEPS_2 * 4);
  memset(world_final, 0, SCR_WIDTH * MAX_STEPS * 4);

  int i;
  unsigned int* palette = (unsigned int*)malloc(255<<2);
  for(i=0; i<255; i++) {
    unsigned int f = i;
    unsigned int f_2 = f >> 1;
    palette[i] = (255 << 24) | (f << 16) | (f_2 << 8);
  }
  ...
  while ( !end) {
    ...
    // Clear screen
    memset(screen_pixels, 0, (SCR_WIDTH*SCR_HEIGHT)*4);
    ...
    //DebugDrawHeightmap(hmap.pixels_2, hmap.width >> 1, hmap.height >> 1, screen_pixels);
    //DrawSamplerBuffer(screen_pixels, world2, SCR_WIDTH_2, MAX_STEPS_2);
